-------------------------------------------------------------------------------
[ Name           ] Sylpheed 3.8beta1-win32
[ Developer      ] Hiroyuki Yamamoto
[ Location       ] https://sylpheed.sraoss.jp/
[ Contact        ] hiro-y@kcn.ne.jp
[ Required       ] Microsoft Windows 8 / 10
...
If you want to use OAuth2 authentication for POP3, IMAP4 or SMTP, you need
OAuth2 client ID settings in oauth2.ini configuration file located in
configuration file folder or the same location of sylpheed.exe
(configuration file folder has priority).
Currently tested with Gmail and MS365.

(Notice) The client IDs described in the oauth2.ini file included in the
package have been created for testing purpose. A warning that these client IDs
are not confirmed by Google or Microsoft will be displayed on the
authentication. There are cases when these client IDs become not available in
the future.

If your mail address and mail server are not described in oauth2.ini,
add entries as below. URIs required for OAuth2 authentication and the creation
method of client IDs are normally open to the public by mail service providers.

(Example: foo@bar.example.com)

[*@bar.example.com]
auth_uri=(authentication URI)
token_uri=(acquiring token URI)
redirect_uri=http://localhost:8089/
client_id=(client ID)
client_secret=(client secret)
scope=(scope (permission) strings)
local_port=8089
